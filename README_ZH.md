...
## 常见问题：
- **Fork 完仓库后，在 Cloudflare 连接账户时看不到自己的仓库，或者点进去 404？**
  因为 GitHub 检测到仓库沿用了本项目的名称，会把你的 GitHub 账号标记（flagged）。账号被标记后，Cloudflare 侧的仓库列表就会看不到这个仓库，或者点进去返回 404。解决办法是在 Fork 时就把仓库名称改成与上游不同的名字（描述一并改掉更稳妥），例如 `2233warden`，以此避开 GitHub 的检测；已经 Fork 好的仓库，可以直接在 GitHub 仓库设置里改名。

- **我删掉重新部署，为什么注册又开始要邀请码了？**
  数据没有跟着删除。需要把 D1 数据库和 KV 空间一并删除，否则旧的用户与邀请码配置仍然留在里面，注册会继续走邀请码校验。

- **我配置了 `JWT_SECRET`，为什么页面还是提示缺少？**
  请配在 Workers 的“设置 → 变量和密钥”（Runtime variables and secrets）里，不要配在“构建”里。构建阶段的变量只在构建时可见，Worker 运行时读不到。

- **为什么升级一次 `JWT_SECRET` 就自动消失了？**
  请把它配置为**机密**，不要用普通文本变量。普通变量在重新构建或部署时可能被覆盖、清空，机密则会持久保留。
